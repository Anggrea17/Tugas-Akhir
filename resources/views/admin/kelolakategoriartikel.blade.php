...
<head>
    ...
    <meta charset="UTF-8">
    <title>Kelola Kategori Artikel - GEMAS</title>
...
<body class="bg-gray-100 text-gray-800" x-data="{ sidebarOpen: false, showAddModal: false, modalEdit: false, editId: '', editNama: '', modalHapus: false, hapusUrl: '', kategoriToDelete: null }">
        ...
        <div class="p-6">
            ...
            <nav class="space-y-2">
                ...
                <a href="{{ route('admin.kategoriartikel') }}"
                    class="flex items-center px-4 py-2 text-gray-700 hover:bg-yellow-100 rounded {{ request()->routeIs('admin.kategoriartikel') ? 'bg-yellow-100 text-gray-700' : '' }}">
                    <svg xmlns="http://www.w3.org/2000/svg"
    ...
    <div class="flex-1 md:ml-64">
        ...
        <div class="max-w-6xl mx-auto mt-8 px-4">
            <div class="flex flex-col md:flex-row justify-between items-center mb-4">
                <h2 class="text-2xl font-semibold text-gray-700">Kelola Kategori Artikel</h2>
                <div class="flex items-center space-x-2 flex-grow md:flex-grow-0">
                    <form method="GET" action="{{ route('admin.kategoriartikel') }}"
                    ...
                    <button @click="showAddModal = true" class="flex items-center gap-2 px-3 py-2 transition">
                        <img src="{{ asset('bahan/plusicon.png') }}" alt="Tambah Kategori Artikel" class="w-8 h-8">
                    </button>
                </div>
            </div>

            <!-- notifikasi kategori artikel -->
            <!-- Success Message -->
            @if (session('success'))
                            ...
                            <tr class="border-t hover:bg-yellow-50">
                                ...
                                <td class="px-4 py-2 space-x-2">
                                    ...
                                    <button
                                        @click="
        editId = '{{ $kategori->id }}';
        editNama = '{{ $kategori->nama_kategori }}';
        modalEdit = true;
    "
                                    ...
                                    {{-- Tombol Hapus --}}
                                    <button
                                        @click="modalHapus = true; hapusUrl = '{{ url('/kategori/' . $kategori->id) }}'; kategoriToDelete = { nama: '{{ addslashes($kategori->nama_kategori) }}' }"
                                    ...
                                    <div class="mt-2">
                                        <p class="text-sm text-gray-500">
                                            ...
                                            <span class="font-medium text-gray-700"
                                                x-text="kategoriToDelete?.nama"></span>?
                                        </p>
                                    </div>
                                </div>
                            </div>
            ...
            <div x-cloak x-show="showAddModal" x-transition:enter="transition ease-out duration-200"
                ...
                <div class="bg-white p-6 rounded-lg w-full max-w-sm shadow-lg relative">
                    ...
                    <h2 class="text-lg font-semibold mb-4 text-yellow-600">Tambah Kategori</h2>
            ...
            <div x-show="modalEdit" x-cloak x-transition
                ...
                <div class="bg-white p-6 rounded-lg w-full max-w-sm shadow-lg relative">
                    ...
                    <!-- Form Edit menggunakan route yang benar dari web.php -->
                    <form method="POST" :action="`{{ url('/kategori') }}/${editId}`">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            <label for="editNama" class="block text-sm font-medium text-gray-700">Nama
                                Kategori</label>
                            <input type="text" id="editNama" name="nama_kategori" x-model="editNama"
                                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:border-yellow-400"
                                required>
                        </div>
